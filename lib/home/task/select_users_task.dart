...
class _SelectUsersPageState extends State<SelectUsersPage>
        ...
        Scaffold(
          ...
          body: Consumer<TaskProvider>(
              builder: (_, TaskProvider taskProvider, __) {
                return WillPopScope(
                  onWillPop: () async {
                    taskProvider.selectedUsers = widget.selectedUsers ?? [];
            ...
            builder: (_, TaskProvider taskProvider, __) {
              return Column(
                ...
                Padding(
                  ...
                  child: CustomButton(
                    text: 'Save', onTap: () {
                    taskProvider.selectedUsers=selectedUsers;
                    runWithLoader(() async {
                      Map<String, dynamic> body = _setParams(taskProvider);
                      await taskProvider.updateTask(body);
                      Navigator.pop(context);
                    });
  ...
  Widget _body(TaskProvider taskProvider) {
    final allUsers =   taskProvider.users;
  ...
  Map<String,dynamic> _setParams(TaskProvider taskProvider){
    List<String> users = taskProvider.selectedUsers.map(
            (user) => user['id'].toString()).toList();
    Map<String, dynamic> body={};
    for(int i=0;i<users.length;i++){
      body.addAll({'users[$i]':users[i]});
    }
    return body;
  }
}
